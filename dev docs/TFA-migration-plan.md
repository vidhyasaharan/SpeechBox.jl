...
|---|---|---|
| Types | `signal` (renamed from `speech_waveform`, briefly `waveform`), `framed_signal`, `spectrum`, `timefreq`, `comp` | `pitch_timefreq`, `RAPT_*`, `GMM`, `Gaussian`, `Categorical`, `filter_coefs`(→ TFA in stage 2) |
...
Back-compat: SpeechBox defines `const speech_waveform = signal` and re-exports it, plus
`@reexport using TimeFrequencyAnalysis`, so pre-migration SpeechBox user code keeps working. (The name
`waveform`, visible only in the short-lived v0.4.0, was dropped in v0.5.0.)
...
| 2026-08-10 | Stage 2 | **Done.** `correlations.jl` (`xcorr(!)`, `acorr(!)`, `acf`, `nacf`) and the filter code from `dsp_utilities.jl` (→ TFA `filters.jl`: `filter_coefs`, `filter_resp`, `filter_magresp` exported; `freq2θ(!)`, `freq2z(!)`, `powers`, `H`, `Hmag` documented internals) moved to TFA with docstrings on every function and three new Documenter pages (Correlations, Filter Responses, Plotting). The `waveform`/`spectrum`/`timefreq` recipes moved to `ext/TimeFrequencyAnalysisRecipesBaseExt.jl` (RecipesBase in `[weakdeps]`; hard deps unchanged); `generate_ticks` lives in TFA `src/plots_support.jl`, imported by SpeechBox for the pitch-overlay recipes. TFA v0.2.0: 97,599 tests green (new correlation/filter/recipe tests on synthetic fixtures; recipes exercised via `RecipesBase.apply_recipe`, no Plots needed), docs build clean. SpeechBox v0.4.1 (compat TFA 0.2): moved files deleted, pitch overlays kept; 5,096 tests green against the dev'ed TFA (the −12 are the moved correlation/Float32 tests); docs build clean; extension verified active when SpeechBox loads (it hard-depends on RecipesBase). **Push TFA before SpeechBox.** |
| 2026-08-11 | Post-stage 2 | **Container renamed: `waveform` → `signal`** in TFA (before the 0.2.0 push, so no released TFA version ever exported `waveform`). All TFA source, extension, tests and docs updated; arguments/locals that were *named* `signal` were renamed (`s`/`samples`) so they cannot shadow the type; the `.signal` **field names of `framed_signal`/`spectrum`/`timefreq` are unchanged**. Test fixtures `signal` → `sig` in both suites (a global named `signal` would collide with the exported type). SpeechBox back-compat: `speech_waveform` **and** `waveform` both remain exported aliases of `signal`. Suites green (TFA 97,599; SpeechBox 5,097 — one extra alias assertion) and both docs build clean. |
| 2026-08-11 | Post-stage 2 | **`waveform` alias dropped from SpeechBox; version → 0.5.0.** The alias only existed because v0.4.0 (public for ~a day) re-exported TFA's `waveform`; no real user code adopted it, and keeping three names for one container worked against the rename's purpose. `speech_waveform` remains the sole back-compat alias of `signal`. Removing a name exported by the pushed v0.4.0 is formally breaking, hence the minor bump. Suite green (5,096). |
