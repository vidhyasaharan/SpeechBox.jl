struct energy_threshold end

struct energy_fraction end


"""
    vad([F, ] sig_frames::framed_signal [; <keyword argument(s)>])

Determine if each frame in the framed\\_signal object `sig_frames` corresponds to voiced speech or not using the VAD algorithm specified by `F` [Default is `energy_threshold()`]. Keyword arguments are specific to algorithm `F`.

"""
vad(sig_frames::framed_signal; params...) = vad(energy_threshold(), sig_frames; params...)


"""
    vad(energy_threshold(), sig_frames::framed_signal [; threshold=0.05])

Estimates the signal 'energy' in each frame (L2 norm computed using the `frame_energy` function) and assigns every frame with 'energy' greater than `threshold` times the maximum frame energy as voiced.
"""
function vad(::energy_threshold, sig_frames::framed_signal; threshold::Float = 0.05)
    energy = frame_energy(sig_frames)
    max_energy = maximum(energy)
    abs_thr = max_energy*threshold
...
"""
    vad(energy_fraction(), sig_frames::framed_signal [; unvoiced_fraction=0.2])
...
"""
function vad(::energy_fraction, sig_frames::framed_signal; unvoiced_fraction::Float = 0.2)
    energy = frame_energy(sig_frames)
